...
namespace CRM.Helper
{
    public class APIOperations
    {
        ...
        public async Task<JObject> GetEntityByID(string entityName, string queryOptions, Guid EntityID)
        {
            HttpRequestMessage GetRequest = new HttpRequestMessage(HttpMethod.Get, getVersionedWebAPIPath() + entityName + "(" + EntityID + ")" + queryOptions);

            string requestURI = httpClient.BaseAddress + getVersionedWebAPIPath() + entityName + queryOptions;

            HttpResponseMessage retrieveResponse = await httpClient.GetAsync(requestURI);

            if (retrieveResponse.StatusCode == HttpStatusCode.OK)
            {
                var retrievedRecordsString = await retrieveResponse.Content.ReadAsStringAsync();
                
                JObject retrievedRecords = JsonConvert.DeserializeObject<JObject>(
                    retrievedRecordsString);
            ...
            else
            {
               ...
               //     retrieveResponse.ReasonPhrase);
                throw new CrmHttpResponseException(retrieveResponse.Content);
            }
        }

        public async Task<JObject> GetAllRecordsForAnEntity(string entityName, string queryOptions)
        {
            HttpRequestMessage GetRequest = new HttpRequestMessage(HttpMethod.Get, getVersionedWebAPIPath() + entityName + queryOptions);

           // GetRequest.Content = new StringContent(contact1.ToString(), Encoding.UTF8, "application/json");

            // HttpResponseMessage retrieveResponse3 = await httpClient.GetAsync(account1Uri + queryOptions);
        ...
        public void ConnectToCRMWithConnectionString(string connectionstring)
        {
            //Create a helper object to read app.config for service URL and application   
            // registration settings.  
            Configuration config = new FileConfiguration(connectionstring, new byte());
		...
		//	handler.Credentials = new NetworkCredential("opr1113", "Suckit92", "IFB");
        ...
        public void ConnectToCRM(String[] cmdargs)
        {
            //Create a helper object to read app.config for service URL and application   
            // registration settings.  
            ...
            if (cmdargs.Length > 0)
            {
                config = new FileConfiguration(cmdargs[0], false);
            }
            else
            {
